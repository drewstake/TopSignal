...
function buildSummary(accountId: number, query: Record<string, DemoQueryValue> | undefined): AccountSummary {
  ...
  return {
    ...
    profit_factor_no_losses: grossLosses.length === 0 && sum(grossWins) > 0,
    profit_factor: grossLosses.length === 0 ? null : round(sum(grossWins) / Math.abs(sum(grossLosses)), 4),
...
function buildDemoBotConfig(accountId: number): BotConfig {
  ...
  return {
    id: 8_100 + suffix,
    name: practice ? "Demo Mathematical Bot — Strategy Lab" : accountId === FOLLOWER_ACCOUNT_ID ? "Demo Mathematical Bot — Copy Account" : "Demo Mathematical Bot — Main Account",
    account_id: accountId,
    provider: "projectx",
    enabled: true,
    execution_mode: "dry_run",
    strategy_type: "topbot_adaptive",
    strategy_params: { revision: "mnq_bayesian_payoff_v1", model_version: "bayesian_cells_v1", horizon_minutes: 15 },
