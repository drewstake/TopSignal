...
export function BotProbabilisticPanel({ research }: { research: BotProbabilisticResearch }) {
  ...
  return <section className="mt-4 rounded-lg border border-app-border p-3" aria-label="Probabilistic research">
    <h4 className="text-sm font-semibold">Probabilistic research · Dry Run shadow</h4>
    <p className="mt-2 text-sm font-medium">NO TRADE · validation incomplete</p>
    <p className="mt-1 text-xs leading-5 text-app-muted">{research.horizon_minutes}-minute horizon · {research.model_version}. Research estimates do not authorize orders or replace the strategy decision above.</p>
    ...
    {forecasts && <p className="mt-2 text-xs text-app-muted">Shadow proposal: {research.research_action.replaceAll("_", " ")}. Routing remains disabled. {research.uncertainty_method}</p>}
