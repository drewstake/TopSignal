...
export function BotProbabilisticPanel({ research, selectedStrategy = false }: { research: BotProbabilisticResearch; selectedStrategy?: boolean }) {
  ...
  return <section className="mt-4 rounded-lg border border-app-border p-3" aria-label="Probabilistic research">
    <h4 className="text-sm font-semibold">{selectedStrategy ? "Selected mathematical strategy · Dry Run" : "Probabilistic research · Dry Run shadow"}</h4>
    <p className="mt-2 text-sm font-medium">{selectedStrategy ? "Model decision inputs · live validation incomplete" : "NO TRADE · validation incomplete"}</p>
    <p className="mt-1 text-xs leading-5 text-app-muted">{research.horizon_minutes}-minute horizon · {research.model_version}. {selectedStrategy ? "This forecast supplies the strategy decision above. Account and risk checks determine the final Dry Run outcome." : "Research estimates do not authorize orders or replace the strategy decision above."}</p>
    ...
    {forecasts && <p className="mt-2 text-xs text-app-muted">{selectedStrategy ? "Model proposal" : "Shadow proposal"}: {research.research_action.replaceAll("_", " ")}. Live routing remains disabled. {research.uncertainty_method}</p>}
