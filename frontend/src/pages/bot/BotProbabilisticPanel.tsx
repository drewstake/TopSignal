...
export function BotProbabilisticPanel({ research, selectedStrategy = false, live = false }: { research: BotProbabilisticResearch; selectedStrategy?: boolean; live?: boolean }) {
  ...
  return <section className="mt-4 rounded-lg border border-app-border p-3" aria-label="Probabilistic research">
    ...
    {forecasts && <p className="mt-2 text-xs text-app-muted">{selectedStrategy ? "Model proposal" : "Shadow proposal"}: {research.research_action.replaceAll("_", " ")}. {selectedStrategy && live ? "Experimental Practice routing; probabilities remain unvalidated." : "Live routing remains disabled."} {research.uncertainty_method}</p>}
