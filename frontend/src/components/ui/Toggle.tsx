...
export function Toggle({ checked, onChange, label, className, disabled, onClick, ...props }: ToggleProps) {
  return (
    <button
      ...
      className={cn(
        "ui-toggle inline-flex min-h-11 items-center gap-2 rounded-xl border px-2.5 py-1.5 text-xs transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-app-accent/45 disabled:cursor-not-allowed disabled:opacity-60 sm:min-h-0",
        checked
