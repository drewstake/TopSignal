...
function MetricCard({ label, value, sub, description }: MetricCardData) {
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-950/30">
      ...
      {sub ? <div className="mt-1 text-xs text-zinc-600 dark:text-zinc-500">{sub}</div> : null}
    </div>
  );
}
  ...
  return (
    <div className="group relative flex-shrink-0">
      <button
        ...
        className="flex h-5 w-5 items-center justify-center rounded-full border border-zinc-300 bg-white text-[10px] font-semibold text-zinc-700 transition hover:border-zinc-400 hover:text-zinc-900 focus:border-zinc-500 focus:text-zinc-900 focus:outline-none dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-200 dark:hover:border-zinc-500 dark:hover:text-zinc-100 dark:focus:border-zinc-400 dark:focus:text-zinc-100"
      >
        i
      </button>
      <div className="absolute right-0 top-6 z-20 hidden w-64 rounded-xl bg-white p-3 text-left text-xs text-zinc-800 shadow-lg ring-1 ring-zinc-200 group-hover:block dark:bg-zinc-900/95 dark:text-zinc-100 dark:ring-zinc-800">
        {description}
      </div>
    </div>
  );
