    ...
    <div className="space-y-3">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950/30 p-4">
          <div className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Time-of-day performance</div>
          <div className="text-xs text-zinc-500">Session ranges shown in New York time.</div>
          <div className="mt-2 divide-y divide-zinc-800 text-sm text-zinc-800 dark:text-zinc-200">
            ...
            {!totals?.timeBlocks?.length ? <div className="py-2 text-zinc-600 dark:text-zinc-400">No realized trades in range.</div> : null}
          </div>
        </div>

        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950/30 p-4">
          <div className="mb-2 flex items-center justify-between text-sm text-zinc-900 dark:text-zinc-100">
            <div>
              <div className="font-semibold">Trade timing</div>
              <div className="text-xs text-zinc-600 dark:text-zinc-400">Precise trade times & PnL (New York time)</div>
            </div>
            <div className="text-xs text-zinc-600 dark:text-zinc-400">{timeAnalysis.busiestTime ? `${timeAnalysis.busiestTime.trades} trades` : "--"}</div>
          </div>

          {loading ? (
            <div className="py-6 text-sm text-zinc-700 dark:text-zinc-300">Loading...</div>
          ) : !timeAnalysis.timeData.length ? (
            <div className="py-6 text-sm text-zinc-700 dark:text-zinc-300">No realized trades to chart.</div>
          ) : (
        ...
        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950/30 p-4">
          <div className="mb-2 flex items-center justify-between text-sm text-zinc-900 dark:text-zinc-100">
            <div>
              ...
              <div className="text-xs text-zinc-600 dark:text-zinc-400">Net PnL by session day</div>
            </div>
            <div className="text-xs text-zinc-600 dark:text-zinc-400">{timeAnalysis.bestDay ? `${timeAnalysis.bestDay.trades} trades` : "--"}</div>
          </div>

          {loading ? (
            <div className="py-6 text-sm text-zinc-700 dark:text-zinc-300">Loading...</div>
          ) : !timeAnalysis.dayData.some((d) => d.trades > 0) ? (
            <div className="py-6 text-sm text-zinc-700 dark:text-zinc-300">No trading days to show.</div>
          ) : (
        ...
        <div className="rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950/30 p-4">
          <div className="mb-2 flex items-center justify-between text-sm text-zinc-900 dark:text-zinc-100">
            <div>
              ...
              <div className="text-xs text-zinc-600 dark:text-zinc-400">PnL by contract</div>
            </div>
            <div className="text-xs text-zinc-600 dark:text-zinc-400">{totals?.instruments.length ?? 0} instrument(s)</div>
          </div>
          <div className="mt-2 divide-y divide-zinc-800 text-sm text-zinc-800 dark:text-zinc-200">
            {(totals?.instruments || []).map((i) => (
              <div key={i.contractId} className="flex items-center justify-between py-2">
                <div className="text-xs text-zinc-600 dark:text-zinc-400">{i.contractId}</div>
                <div className="text-right">
            ...
            {!totals?.instruments?.length ? <div className="py-2 text-zinc-600 dark:text-zinc-400">No realized trades in range.</div> : null}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950/30 p-4">
        <div className="mb-2 flex items-center justify-between text-sm text-zinc-900 dark:text-zinc-100">
          <div>
            ...
            <div className="text-xs text-zinc-600 dark:text-zinc-400">Cumulative net PnL with daily PnL overlay</div>
          </div>
          <div className="text-xs text-zinc-600 dark:text-zinc-400">Range: last {effectiveDaysBack} day(s)</div>
        </div>

        {loading ? (
          <div className="py-6 text-sm text-zinc-700 dark:text-zinc-300">Loading...</div>
        ) : !equity.length ? (
          <div className="py-6 text-sm text-zinc-700 dark:text-zinc-300">No equity data found for this range.</div>
        ) : (
