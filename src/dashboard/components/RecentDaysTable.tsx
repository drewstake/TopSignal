...
export default function RecentDaysTable({ loading, computed }: RecentDaysTableProps) {
  return (
    <div className="mt-4 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950/30">
      <div className="grid grid-cols-12 bg-zinc-100 px-4 py-2 text-xs text-zinc-600 dark:bg-zinc-950/40 dark:text-zinc-400">
      ...
      <div className="divide-y divide-zinc-200 bg-white dark:divide-zinc-800 dark:bg-zinc-950/20">
        {loading ? (
          ...
          <div className="px-4 py-4 text-sm text-zinc-600 dark:text-zinc-300">No day data found for this range.</div>
        ) : (
          computed.days
            .slice()
            .reverse()
            ...
            .map((d) => (
              <div key={d.date} className="grid grid-cols-12 items-center px-4 py-3 text-sm text-zinc-800 dark:text-zinc-200">
                <div className="col-span-3 text-zinc-500 dark:text-zinc-300">{d.date}</div>
