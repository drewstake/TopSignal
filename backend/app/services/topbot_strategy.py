...
def evaluate(candles):
    ...
    if short_setup:
        ...
        if not short_entry_allowed:
            return hold(
                "TopBot long bias: short entry blocked until the 20 EMA is below "
                "a falling 50 EMA. Existing positions retain their stop and target."
            )
    ...
    # Absolute targets prevent entry signals from acting as flattening deltas.
    # The shared router checks the authoritative provider position: already at
    # this target is a no-op; crossing an existing position is rejected. Replay
    # uses the same target semantics. Never emit an opposite-signal exit.
    payload.update(signal_category="entry", target_position_qty=float(direction * RULES["position_size"]),
                   exit_policy=RULES["exit_policy"])
    return indicators.SignalResult(
